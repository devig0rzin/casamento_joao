...
import { setGiftStatus } from "@/lib/local-store";
import { buildPixPayload, formatCurrency } from "@/lib/pix";
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_390px]">
      ...
      <aside className="glass-panel sticky top-24 h-fit rounded-[2rem] p-6">
        ...
        <textarea className="mt-5 min-h-28 w-full rounded-2xl border border-rosewood/10 bg-white p-4 text-xs text-rosewood/70" readOnly value={pixPayload} aria-label="Pix copia e cola" />
        <button className="focus-ring mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-rosewood px-6 font-bold text-white shadow-soft transition hover:-translate-y-0.5" type="button" onClick={copyPix}>
          <Copy size={18} /> {copied ? "Copiado" : "Copiar Pix"}
        </button>
        <p className="mt-4 text-sm leading-6 text-rosewood/62">Depois do pagamento, envie o comprovante para os noivos. A confirmacao automatica fica pronta para integrar com provedor Pix/Supabase.</p>
      </aside>
    </div>
  );
}
