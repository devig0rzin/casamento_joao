...
import { loadGiftPayments, loadGuests } from "@/lib/local-store";
import { formatCurrency } from "@/lib/pix";
import { gifts, type GiftPayment, type Guest } from "@/lib/wedding-data";
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    loadGiftPayments().then(setPayments);
  }, [authenticated]);

  const filteredGuests = guests.filter((guest) => `${guest.name} ${guest.email} ${guest.phone}`.toLowerCase().includes(query.toLowerCase()));
  const filteredPayments = payments.filter((payment) => `${payment.giftName} ${payment.pixTxid} ${payment.status}`.toLowerCase().includes(query.toLowerCase()));
  const totalPeople = guests.reduce((sum, guest) => sum + guest.companions + 1, 0);
  const giftTotal = payments.reduce((sum, payment) => sum + payment.amount, 0);
    ...
    <section className="container-premium pt-32">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-4">
        {[
          ...
          { icon: Gift, label: "Pix gerados", value: payments.length },
          { icon: Gift, label: "Valor registrado", value: formatCurrency(giftTotal) },
      ...
      <article className="glass-panel mt-8 rounded-[2rem] p-6">
        <h2 className="font-bold text-rosewood">Pagamentos Pix registrados</h2>
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="text-xs uppercase tracking-[0.18em] text-rosewood/45">
              <tr>
                <th className="py-3">Presente</th>
                <th>Valor</th>
                <th>Status</th>
                <th>TXID Pix</th>
                <th>Data</th>
            ...
            <tbody>
              {filteredPayments.map((payment) => (
                <tr className="border-t border-rosewood/10" key={payment.id}>
                  <td className="py-3 font-bold text-rosewood">{payment.giftName}</td>
                  <td>{formatCurrency(payment.amount)}</td>
                  <td>{payment.status}</td>
                  <td>{payment.pixTxid}</td>
                  <td>{new Date(payment.createdAt).toLocaleString("pt-BR")}</td>
                </tr>
              ))}
              {!filteredPayments.length ? (
                <tr className="border-t border-rosewood/10">
                  <td className="py-4 text-rosewood/60" colSpan={5}>
